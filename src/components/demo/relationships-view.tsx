...
export function RelationshipsView({ onContinue }: { onContinue: (names: string[]) => void }) {
  ...
  return (
    <div className="font-grotesque relative h-full w-full overflow-hidden bg-white text-black">
      ...
      <div className="absolute left-[8.6%] top-[18%] md:top-[23.5%]">
        <h1 className="text-2xl font-bold leading-tight tracking-[-0.015em] md:[font-size:3.3vw]">
          let&apos;s get to know you.
        </h1>
        <p key={phase} className="mt-2 text-sm text-black/75 [animation:bramble-fade_400ms_ease-out] md:[font-size:1.06vw]">
          {SUBTITLE[phase]}
        </p>
      </div>
        ...
        {ROSTER.map((person, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                className="mt-[8%] whitespace-nowrap text-[10px] transition-opacity duration-500 md:[font-size:1.15vw]"
                style={{ opacity: settled ? 1 : 0 }}
              >
                {person.name}
              </span>
      ...
      <div className="absolute bottom-[10%] right-[6%] md:bottom-[15.5%] md:right-[4.6%]">
        {phase === "idle" && (
...
function BlackButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`px-5 py-3 text-sm font-bold text-white transition-opacity md:[font-size:1.52vw] md:[padding:1.32vw_2.05vw] ${
