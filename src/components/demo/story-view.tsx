...
export function StoryView({ scenario, onBack }: { scenario: Scenario; onBack: () => void }) {
  const script = useMemo(() => sequenceFor(scenario.id, scenario.history.at(-1)?.text), [scenario]);
  ...
  return (
    ...
    >
      {/* Chrome */}
      <div className="absolute top-0 left-0 z-20 flex items-center gap-3 p-5 md:gap-[1.2vw] md:p-[2.8vw]">
        <button
          type="button"
          onClick={onBack}
      ...
      {/* Right — the story */}
      <div
        className="flex-1 overflow-y-auto px-6 pb-10 transition-opacity duration-250 md:h-full md:w-[56%] md:py-[5vw] md:pr-[5.5vw] md:pl-0"
        ...
        <div className="flex flex-col gap-4 md:gap-[1.6vw]">
          ...
          {beat.incomingMessage && (
            <div className="flex flex-col gap-1.5 pl-2 md:gap-[0.7vw] md:pl-[0.8vw]">
              <span className="text-[10px] font-semibold tracking-[0.14em] text-black/45 uppercase md:[font-size:0.92vw]">
                {scenario.personaName}
              </span>
              {isEmail ? (
                <p className="border-l-2 border-black/20 pl-3 text-sm text-black/75 italic md:[padding-left:1.1vw] md:[font-size:1.35vw]">
                  {beat.incomingMessage}
                </p>
              ) : (
                <p className="w-fit rounded-lg bg-[#e0e0e0] px-3 py-2 text-sm text-black/85 md:[border-radius:1.1vw] md:[padding:0.9vw_1.2vw] md:[font-size:1.35vw]">
                  {beat.incomingMessage}
                </p>
              )}
            </div>
...
function ChoiceRow({ choice, onSelect }: { choice: StoryChoice; onSelect: () => void }) {
  return (
    <button
      type="button"
      onClick={onSelect}
      className="group flex w-full cursor-pointer items-center gap-2 rounded-full border border-black/15 px-4 py-2.5 text-left text-sm text-black/80 transition-colors duration-150 hover:border-black hover:bg-black hover:text-white md:[height:3.6vw] md:gap-[0.8vw] md:[padding:0_1.8vw] md:[font-size:1.25vw]"
    >
...
function EndOfChapter({ onBack, onReplay }: { onBack: () => void; onReplay: () => void }) {
  return (
    <div className="flex flex-col gap-3 md:gap-[1.2vw]">
      <span className="text-[10px] font-semibold tracking-[0.18em] text-black/40 uppercase md:[font-size:1.05vw]">
        end of chapter
      </span>
      <div className="flex flex-wrap gap-2 md:gap-[1vw]">
        <button
          type="button"
          onClick={onBack}
          className="cursor-pointer rounded-full bg-black px-5 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-85 md:[height:3.6vw] md:[padding:0_2vw] md:[font-size:1.25vw]"
        >
          Back to your world
        </button>
        <button
          type="button"
          onClick={onReplay}
          className="cursor-pointer rounded-full border border-black/15 px-5 py-2.5 text-sm text-black/80 transition-colors hover:border-black/40 md:[height:3.6vw] md:[padding:0_2vw] md:[font-size:1.25vw]"
        >
          Play again
        </button>
      </div>
